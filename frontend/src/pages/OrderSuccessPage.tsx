          ...
          <div className="p-6 sm:p-8 space-y-6">
                ...
                {order.items?.map((item: any, idx: number) => (
                  <li key={idx} className="py-3 flex justify-between items-center text-sm gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="text-xs text-gray-400">Qty: {item.quantity} · PKR {Number(item.unit_price).toLocaleString()}</span>
                    </div>
                    <span className="font-bold text-gray-900">PKR {Number(item.line_total).toLocaleString()}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="pt-4 border-t border-purple-50 space-y-2.5 text-sm max-w-sm ml-auto">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal</span>
                <span className="font-medium text-gray-900">PKR {Number(order.total).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>Shipping</span>
              ...
              <div className="border-t border-purple-50 pt-2 flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-purple-600">PKR {Number(order.total).toLocaleString()}</span>
              </div>
            </div>
          </div>
        </section>
