      ...
      >
          ...
          {cartItems.map((item: any) => (
            <li key={item.id} className="py-3 flex items-center justify-between text-sm gap-2" style={{ borderColor: 'var(--border-color)' }}>
              <div className="min-w-0 flex-1">
                ...
                <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Qty: {item.quantity} · PKR {Number(item.product.price).toLocaleString()}</span>
              </div>
              <span className="font-bold" style={{ color: 'var(--text-primary)' }}>PKR {(Number(item.product.price) * item.quantity).toLocaleString()}</span>
            </li>
          ))}
        </ul>
        ...
        <div className="pt-4 space-y-3 text-sm" style={{ borderTop: '1px solid var(--border-color)' }}>
          <div className="flex justify-between">
            ...
            <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>PKR {subtotal.toLocaleString()}</span>
          </div>
          {discount > 0 && (
            <div className="flex justify-between">
              <span style={{ color: 'var(--text-secondary)' }}>Discount</span>
              <span className="font-semibold" style={{ color: '#22c55e' }}>-PKR {discount.toLocaleString()}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span style={{ color: 'var(--text-secondary)' }}>Tax (8%)</span>
            <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>PKR {tax.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="pt-3 flex justify-between items-center" style={{ borderTop: '1px solid var(--border-color)' }}>
            ...
            <span className="text-xl font-bold" style={{ color: 'var(--text-accent)' }}>PKR {total.toLocaleString()}</span>
          </div>
        </div>
